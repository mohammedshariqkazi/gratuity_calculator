...
const App = () => {
  ...
  const calculateGratuity = () => {
    ...
    setTimeout(() => {
      ...
      const years = differenceInMilliseconds / (1000 * 60 * 60 * 24 * 365); // Calculate prorated years of service
      const gratuity = gratuityDues.reduce((sum, dues) => sum + dues, 0) * basicSalary * years; // Adjust the calculation with prorated years
    ...
    <div className="container">
      ...
      <div>
        ...
        <div className="dues-container">
          {Array.from({ length: gratuityDues.length + 1 }, (_, index) => (
            <input
              type="number"
              key={index}
      ...
      {gratuityAmount !== null && (
        <div id="result">
          ...
          <p>{`Calculated based on ${gratuityDues.length} year(s) of service (${gratuityDues.join(' days, ')} days)`}</p>
        </div>
      )}
      <footer>
